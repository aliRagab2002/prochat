...
const Patiant = require("../models/patiant.model");
...
router.post('/patiantA', (req, res) => {
    console.log(req.body);

    if (req.body.hospital === "Hospital A") {
        const patiantA = new Patiant(req.body);
        patiantA.save()
            .then((patiantA) => {
                res.send(patiantA);
            })
...
router.get("/patiantA",asyncwrapper( async(req, res, next)=>{
...
router.patch('/patiantA/:id',async(req,res)=>{
    if (req.body.hospital === "Hospital A"){
...
router.delete('/patiantA/:id',async(req,res)=>{
    try{
        const _id = req.params.id

        const new_doner = await Patiant.findByIdAndDelete(_id)
